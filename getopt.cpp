...
static int _getopt_internal(int argc, char *const *argv, const char *optstring, const struct option *longopts,
            ...
            if (*nameend) {
                ...
                if (pfound->has_arg)
                    ...
                    if (opterr) {
                        if (argv[optind - 1][1] == '-')
                            /* --option */
                            fprintf(stderr, gettext("%s: option `--%s' doesn't allow an argument\n"), argv[0],
                                    pfound->name);
                    } else {
                        /* +option or -option */
                        fprintf(stderr, gettext("%s: option `%c%s' doesn't allow an argument\n"), argv[0],
                ...
                if (optind < argc)
                    ...
                    if (opterr)
                        fprintf(stderr, gettext("%s: option `%s' requires an argument\n"), argv[0], argv[optind - 1]);
                    nextchar += strlen(nextchar);
                    return optstring[0] == ':' ? ':' : '?';
                }
            }
        ...
        if (temp[1] == ':') {
            if (temp[2] == ':') {
                ...
                if (*nextchar != '\0') {
                    ...
                    if (opterr) {
                        /* 1003.2 specifies the format of this message.  */
                        fprintf(stderr, gettext("%s: option requires an argument -- %c\n"), argv[0], c);
                    }
                    optopt = c;
                    if (optstring[0] == ':')
                        c = ':';
